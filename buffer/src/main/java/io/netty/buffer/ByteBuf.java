 ...
 *
 *
 * 如果底层实现bytebuf的是堆内存那么可以通过array获取底层的字节数组,如果是对外内存那么只能通过Niobytebuffer来访问;
 * 这个bytebuf就是对以上两种或更多种的抽象;
 */
public abstract class ByteBuf implements ReferenceCounted, Comparable<ByteBuf> {
     ...
     *
     * 调整bytebuf的容量大小,如果新的容量大小小于原始容量的大小,那么会截断bytebuf,如果大于原始容量那么会进行扩展;
     * 如果大于最大容量限制那么抛异常;
     */
     ...
     *
     * 返回该bytebuf的字节序;
     * ByteOrder是用来表示大端字节序还是低端字节序,意思是比如int类型的高位在前还是在后;
     */
    @Deprecated
     ...
     * instead of creating a buffer with swapped {@code endianness}.
     *
     * 按指定字节序返回bytebuf,如果字节序和this的字节序不同那么会进行修改并且readerIndex和writerIndex可能会受影响;
     * 如果字节序相同那么就返回this即可无需做改变;
     */
    @Deprecated
     ...
     *
     * 如果此缓冲区是另一个缓冲区的包装器，则返回基础缓冲区实例,如果不是包装了一个bytebuf那么就返回null;
     */
     ...
     *
     * 设置写索引如果小于读索引或者大于bytebuf的最大容量那么抛异常;
     */
     ...
     *
     * 返回在不涉及内部重新分配或数据副本的情况下可以写入的最大字节数;
     * ???question:没弄懂???
     *
     * 如果不扩容能提供的可写大小,默认capacity - writeindex也可以是 maxcapacity - writeindex;
     */
     ...
     *
     * 清空bytebuf,将readerIndex和writerIndex置为0,这个方法等效setIndex(0, 0);
     */
     ...
     * consumption.
     *
     * 丢弃掉已读过的大小(这个大小取决于其内部实现，以潜在的额外内存消耗为代价降低总体内存带宽消耗) < readerIndex - 0;
     */
     ...
     * @see #capacity(int)
     *
     * 这个方式用来确保这个指定大小的写空间,如果当前可写大小大于这个指定值那么不做任何处理,如果小于就得进行扩容或者抛异常等;
     */
     ...
     *
     * 这个和上面相同只不过多了一个force,如果为true那么并且最大maxCapacity不够用了,那么进行强制的扩容;
     */
     ...
     * 从指定索引处获取一个布尔值,注意不会修改读写索引并且指定的索引不能越界,不能大于最大索引不能小于0;
     */
     ...
     *         {@code index + 1} is greater than {@code this.capacity}
     *
     * 从指定索引处获取一个byte值,注意不会修改读写索引并且指定的索引不能越界,不能大于最大索引不能小于0;
     */
     ...
     *         {@code index + 1} is greater than {@code this.capacity}
     *
     * 从指定索引处获取一个无符号的byte值,注意不会修改读写索引并且指定的索引不能越界,不能大于最大索引不能小于0;
     */
     ...
     *
     * 从指定索引处获取一个short值,注意不会修改读写索引并且指定的索引不能越界,不能大于最大索引不能小于0;
     */
     ...
     *         {@code index + 2} is greater than {@code this.capacity}
     *
     * 从指定索引处获取一个低位字节序的short值,注意不会修改读写索引并且指定的索引不能越界,不能大于最大索引不能小于0;
     */
     ...
     *         {@code index + 2} is greater than {@code this.capacity}
     *
     * 从指定索引处获取一个无符号的short值,注意不会修改读写索引并且指定的索引不能越界,不能大于最大索引不能小于0;
     */
     ...
     *         {@code index + 2} is greater than {@code this.capacity}
     *
     * 从指定索引处获取一个高位字节序的无符号short值,注意不会修改读写索引并且指定的索引不能越界,不能大于最大索引不能小于0;
     */
     ...
     *
     * 从指定索引处获取一个24位的int值,注意不会修改读写索引并且指定的索引不能越界,不能大于最大索引不能小于0;
     */
     ...
     *         {@code index + 3} is greater than {@code this.capacity}
     *
     * 从指定索引处获取一个低位字节序的24位的int值,注意不会修改读写索引并且指定的索引不能越界,不能大于最大索引不能小于0;
     */
     ...
     *         {@code index + 3} is greater than {@code this.capacity}
     *
     * 从指定索引处获取一个无符号的24位的int值,注意不会修改读写索引并且指定的索引不能越界,不能大于最大索引不能小于0;
     */
     ...
     *         {@code index + 3} is greater than {@code this.capacity}
     *
     * 从指定索引处获取一个低位字节序的无符号的24位的int值,注意不会修改读写索引并且指定的索引不能越界,不能大于最大索引不能小于0;
     */
     ...
     *
     * 从指定索引处获取一个int值,注意不会修改读写索引并且指定的索引不能越界,不能大于最大索引不能小于0;
     */
     ...
     *         {@code index + 4} is greater than {@code this.capacity}
     *
     * 从指定索引处获取一个低位字节序int值,注意不会修改读写索引并且指定的索引不能越界,不能大于最大索引不能小于0;
     */
     ...
     *         {@code index + 4} is greater than {@code this.capacity}
     *
     * 从指定索引处获取一个无符号的int值,注意不会修改读写索引并且指定的索引不能越界,不能大于最大索引不能小于0;
     */
     ...
     *         {@code index + 4} is greater than {@code this.capacity}
     *
     * 从指定索引处获取一个无符号的低位字节序的int值,注意不会修改读写索引并且指定的索引不能越界,不能大于最大索引不能小于0;
     */
     ...
     *
     * 从指定索引处获取一个long值,注意不会修改读写索引并且指定的索引不能越界,不能大于最大索引不能小于0;
     */
     ...
     *         {@code index + 8} is greater than {@code this.capacity}
     *
     * 从指定索引处获取一个低位字节序的long值,注意不会修改读写索引并且指定的索引不能越界,不能大于最大索引不能小于0;
     */
     ...
     *         {@code index + 2} is greater than {@code this.capacity}
     *
     * 从指定索引处获取一个char值,注意不会修改读写索引并且指定的索引不能越界,不能大于最大索引不能小于0;
     */
     ...
     *         {@code index + 4} is greater than {@code this.capacity}
     *
     * 从指定索引处获取一个float值,注意不会修改读写索引并且指定的索引不能越界,不能大于最大索引不能小于0;
     */
     ...
     *         {@code index + 4} is greater than {@code this.capacity}
     *
     * 从指定索引处获取一个低位字节序的float值,注意不会修改读写索引并且指定的索引不能越界,不能大于最大索引不能小于0;
     */
     ...
     *         {@code index + 8} is greater than {@code this.capacity}
     *
     * 从指定索引处获取一个double值,注意不会修改读写索引并且指定的索引不能越界,不能大于最大索引不能小于0;
     */
     ...
     *         {@code index + 8} is greater than {@code this.capacity}
     *
     * 从指定索引处获取一个低位字节序的double值,注意不会修改读写索引并且指定的索引不能越界,不能大于最大索引不能小于0;
     */
     ...
     *            {@code this.capacity}
     *
     * 将此缓冲区的数据从指定的绝对{@code index}开始传输到指定的目标,知道目标不可写为止;
     */
     ...
     *
     * 将此缓冲区的数据从指定的绝对{@code index}开始传输指定长度到指定的目标;
     */
     ...
     *            {@code dst.capacity}
     *
     * 将此缓冲区的数据从指定的绝对{@code index}开始从目标指定index传输指定长度到指定的目标;
     */
     ...
     *            {@code this.capacity}
     *
     * 将此缓冲区的数据从指定的绝对{@code index}开始传输到目的byte[]中;
     */
     ...
     *         if {@code length} is greater than {@code this.readableBytes}
     *
     * the newly created buffer,返回从readerIndex开始到指定长度的字节,readerIndex会进行更新,注意越界限制;
     */
     ...
     *         {@code -1} otherwise.
     *
     * 返回从指定开始到结束的位置之间第一个出现的value值的位置, 如果from大于to那么按相反顺序执行;
     */
     ...
     *
     * 返回从readerIndex到writerIndex第一个出现value的位置;
     */
     ...
     * reference count will NOT be increased.
     *
     * 返回bytebuf的可读部分的字节,return的buf和this的buf会互相影响但是读写索引是各自隔离的,此方法不会修改读写索引,也不会增加引用计数;
     */
     ...
     * a buffer implementation that produces less garbage.
     *
     * 返回bytebuf的可读部分的字节,return的buf和this的buf会互相影响但是读写索引是各自隔离的,此方法不会修改读写索引,这个会增加引用计数;
     */
     ...
     * reference count will NOT be increased.
     *
     * 返回bytebuf的可读部分的字节指定了起始索引和长度,return的buf和this的buf会互相影响但是读写索引是各自隔离的,此方法不会修改读写索引,也不会增加引用计数;
     */
     ...
     *
     * 返回和本身一样的buf但是读写索引是隔离的但内容是互相影响,不会增加引用计数;
     */
     ...
     * a buffer implementation that produces less garbage.
     *
     * 返回和本身一样的buf但是读写索引是隔离的但内容是互相影响,这个会增加引用计数;
     */
     ...
     *
     * 返回包含原生byteBuffer的数量,只会返回-1或大于0的数;
     */
     ...
     * @see #nioBuffers(int, int)
     *
     * 将可读的部分封装为原生bytebuffer返回,不会和本buf互相影响{???question:到底影响还是不影响???};
     * {
     *    tip: 动态缓冲区和静态缓冲区,静态缓冲区需要程序员自己管理的回收,并且缓冲内容一直在在未清空之前所以多线程循环没问题但可能会发生内存泄漏.
     *         而动态的可能会发生覆盖,所以多线程执行循环等要小心,而且内存的回收不需要要程序员自己管理;
     * }
     */
     ...
     * {@link #arrayOffset()}.
     *
     * 判断buf的字节数组是否支持, 如果底层实现bytebuf的是堆内存那么可以通过array获取底层的字节数组,如果是对外内存那么只能通过Niobytebuffer来访问;
     */
     ...
     *
     * 返回buf的字节数组,如果底层实现bytebuf的是堆内存那么可以通过array获取底层的字节数组,如果是对外内存那么只能通过Niobytebuffer来访问;
     */
     ...
     *         if there no accessible backing byte array
     *
     * 返回buf的字节组的起始偏移量,如果底层实现bytebuf的是堆内存那么可以通过array获取底层的字节数组,如果是对外内存那么只能通过Niobytebuffer来访问;
     */
     ...
     *
     * 是否可以访问的,内部使用,如果引用计数为0那么可能对应的内存已经回收了;
     * */
