...
public abstract class AbstractByteBuf extends ByteBuf {
    ...
    // 如果为true只有引用计数大于0,那么该buf才能可用;
    static final boolean checkAccessible; // accessed from CompositeByteBuf
    // 更改或者获取读写索引是否需要进行越界检查例如readerIndex(index) readindex <= index <= writeindex;
    ...
    final void ensureWritable0(int minWritableBytes) {
        ...
        // 如果要写大小没有越界,那么进行access检查;
        if (targetCapacity <= capacity()) {
            ensureAccessible();
            return;
        }
        // 如果越界那么看是否可以扩容,如果不可以那么抛异常;
    ...
    // return 0: capacity - writeindex > minWritableBytes;
    // return 1: force = false && minWritableBytes > maxCapacity - writerIndex;
    // return 2: minWritableBytes <= maxCapacity - writerIndex;
    // return 3: capacity != maxCapacity;
    @Override
     ...
     *
     * 判断是否可以访问 checkAccessible = true && 该对象的引用计数是否大于零;
     */
