...
abstract class PooledByteBuf<T> extends AbstractReferenceCountedByteBuf {
    ...
    // 底层存储实现(byte[] || byteBuffer);
    protected T memory;
    // 相对于memory的偏移量;
    protected int offset;
    // buf的大小,和offset来确定在memory中的位置;
    protected int length;
    // 表示buf的大小 = length(真实申请的空间大小reqCapacity) + 空闲的(真实分配的空间大小normCapacity);
    int maxLength;
     ...
     * 重置buf为初始状态,设置引用数为0和读写索引为0;
     * 一般用于把回收的对象重新使用时调用,从而确保状态正确;
     */
