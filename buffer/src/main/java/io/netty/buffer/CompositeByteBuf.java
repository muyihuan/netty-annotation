 ...
 * 建议使用{@link//bytebuffallocator#compositeBuffer()}或{@link//unpoled#wrappedBuffer(ByteBuf…)}而不是显式调用构造函数
 *
 * 组合bytebuf,多个缓冲区显示为单个合并缓冲区的虚拟缓冲区,可能包含多个堆外内存和多个堆内存;
 *
 * ???question:还没看???
 */
public class CompositeByteBuf extends AbstractReferenceCountedByteBuf implements Iterable<ByteBuf> {
    ...
    // Component 大小: max(maxNumComponents, initComponents)
