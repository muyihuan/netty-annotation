...
final class DefaultChannelHandlerContext implements ChannelHandlerContext, ResourceLeakHint {
     ...
     *
     * 表示这个context还没有被调用,执行不同的操作状态也会随之改变;
     */
    ...
    /**
     * 通过MASK_REGISTER来查找可以进行处理注册的handerContext(handerContext主要包含了name&hander),
     * hander重写了channelHander的那个方法，也就拥有了这个方法的标志位，然后就可以通过标志位来查找某个功能的hander;
     *
     * {
     *    tip:非常巧妙,可以学习;
     * }
    ...
    /**
     * 从tail向前遍历，如果没有匹配的，最终返回head节点，说明head节点可能需要支持channel的全部功能;
     */
